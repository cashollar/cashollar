...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::CSO)
    {
    }
                                              ...
                                              filter(0)
{
    ...
    ui->pushButton_Discord->setStatusTip(tr("节点挖矿"));
...
//    ui->label_csocoin4->setVisible(showImmature || showWatchOnlyImmature);
...
//        model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
        ...
        connect(ui->blabel_csocoin, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

        emit model->makeBalance();
    }

    // update the display unit, to not use the default ("csocoin")
    updateDisplayUnit();
}
...
void OverviewPage::updateMasternodeInfo()
{
  ...
  if (masternodeSync.IsBlockchainSynced() && masternodeSync.IsSynced())
  {
   ...
   std::vector<CMasternode> vMasternodes = mnodeman.GetFullMasternodeMap();
    ...
    if (CurrentBlock >= 0) {
        ...
        ui->roi_12->setText(mn1==0 ? " " : QString::number(  50000/roi1,'f',1).append(" days"));
        ui->roi_22->setText(mn2==0 ? " " : QString::number( 150000/roi2,'f',1).append(" days"));
        ui->roi_32->setText(mn3==0 ? " " : QString::number( 300000/roi3,'f',1).append(" days"));
        ui->roi_42->setText(mn4==0 ? " " : QString::number( 500000/roi4,'f',1).append(" days"));
    }
    CAmount tNodesSumm = mn1*50000 + mn2*150000 + mn3*300000 + mn4*500000;
  ...
  if (CurrentBlock >= 0) {
      ui->label_lcolat->setText("50000 CSO");
      ui->label_mcolat->setText("150000 CSO");
      ui->label_fcolat->setText("300000 CSO");
      ui->label_pcolat->setText("500000 CSO");
  }
...
void OverviewPage::updateBlockChainInfo()
{
    if (masternodeSync.IsBlockchainSynced())
    {
        ...
        double BlockReward = GetBlockValue(CurrentBlock);
        double BlockRewardcsocoin =  static_cast<double>(BlockReward/COIN);
        ...
        ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardcsocoin, 'f', 1).append(" | ") + QString::number(GetSporkValue(SPORK_11_DEV_FEE)).append("%"));

        ui->label_Supply_value->setText(QString::number(chainActive.Tip()->nMoneySupply / COIN).append(" CSO"));
...
void OverviewPage::on_pushButton_Website_clicked() {
    QDesktopServices::openUrl(QUrl("http://www.cashollar.com", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Discord_clicked() {
    QDesktopServices::openUrl(QUrl("http://www.nodemn.com/toHostMn?coinId=858dde27cd914aa0a1e08cd3cac24b7d", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Telegram_clicked() {
    QDesktopServices::openUrl(QUrl("https://i.bishijie.com/home/247008629/dynamic", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Twitter_clicked() {
    QDesktopServices::openUrl(QUrl("https://jq.qq.com/?_wv=1027&k=5FwDUpY", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Reddit_clicked() {
    QDesktopServices::openUrl(QUrl("http://ico.cashollar.com/Login/index", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Medium_clicked() {
    QDesktopServices::openUrl(QUrl("http://cashollar.com/Data/index.html", QUrl::TolerantMode));
}
/*
...
void OverviewPage::on_pushButton_Explorer_clicked() {
    QDesktopServices::openUrl(QUrl("https://explorer.cashollar.com", QUrl::TolerantMode));
}
